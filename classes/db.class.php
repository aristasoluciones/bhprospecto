...
class DB
{
    ...
    function GetResultWhitRealTotalRows() {
        $result = array();
        $this->ExecuteQuery();

        $sQuery = "SELECT FOUND_ROWS()";
        $rsReal = mysqli_query($this->conn_id, $sQuery) or die (trigger_error(mysqli_error($this->conn_id)));
        $asReal = mysqli_fetch_array($rsReal);
        $result['filteredTotal'] = $asReal[0];
        while ($rs = mysqli_fetch_assoc($this->sqlResult)) {
            $retArray[] = $rs;
        }

        $this->CleanQuery();
        $result['result'] = $retArray;
        return $result;
    }
