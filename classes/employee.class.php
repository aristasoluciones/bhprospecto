...
class Employee extends User
{
	...
	protected $selectables = ['id', 'name', 'phone', 'email', 'active', 'phone_extension', 'celphone'];
	...
	public function enumerate() {
        $agregates = $this->Util()->handleRequest($this->selectables, $_REQUEST);
        $orderBy = $agregates['orderBy'];
        $limit = $agregates['limit'];
        $columnas  =  implode(',', $this->selectables);
        $table = $this->table;

       $sQuery = "SELECT SQL_CALC_FOUND_ROWS ".str_replace(" , ", " ", $columnas)."
                   FROM  $table	
                   $orderBy
                   $limit
                   ";
		$this->Util()->DB()->setQuery($sQuery);
		$records = $this->Util()->DB()->GetResultWhitRealTotalRows();

		$indexColumn =  $this->indexColumn;
		$sQuery =" SELECT COUNT($indexColumn) from $table";
        $this->Util()->DB()->setQuery($sQuery);
        $total = $this->Util()->DB()->GetSingle();

        $data['draw'] = $_REQUEST['draw'];
        $data['recordsTotal'] =  $total;
        $data['recordsFiltered'] =  $records['filteredTotal'];
