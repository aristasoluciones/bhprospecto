...
class Role extends Main
{
    ...
    function getPermisosByRol(){
        if($this->isAdmin())
            $sql =  "SELECT permission_id from role_permission where 1";
        else
            $sql =  "SELECT permission_id from role_permission where rol_id='".$this->rolId."' ";
        $this->Util()->DB()->setQuery($sql);
        $array_perm = $this->Util()->DB()->GetResult();
        $owns_lineal =$this->Util()->ConvertToLineal($array_perm,'permission_id');
        return $owns_lineal;
    }
