...
class Util extends CustomError
{
    ...
    function handleRequest($columnas, $request) {

        $req['limit'] = (isset($request['start']) && $request['length'] !== '-1') ? " LIMIT ".$request['start'].", ".$request['length'] : "";

        $req ['orderBy'] = "ORDER BY  ";
        if (isset($request['order'])) {

            $colsOrder = !is_array($request['order']) ? [] : $request['order'];
            foreach ($colsOrder as $col => $colum) {
                $req['orderBy'] .=  $columnas[$colum['column']] ." ".$colum['dir'].", ";
            }
            $req['orderBy'] = substr_replace($req['orderBy'],"", -2);
            if ($req['orderBy'] === 'ORDER BY')
                $req['orderBy'] = "" ;
        }
        return $req;
    }
